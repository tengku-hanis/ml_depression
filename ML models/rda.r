# ML models - regularised discriminant model
...
discrimreg_wf <- 
  workflow() %>% 
  add_recipe(discrimreg_rec) %>% 
  add_model(discrimreg_mod)
...
# Save CV result
# readr::write_rds(discrimreg_tuned, "full_data_analysis_7Dec2025/cv_results/30features/discrimreg_cv_result.rds")
...
# Plot
autoplot(discrimreg_tuned)
autoplot(discrimreg_tuned, metric = "sens")
autoplot(discrimreg_tuned, metric = "spec")
autoplot(discrimreg_tuned, metric = "pr_auc")
autoplot(discrimreg_tuned, metric = "f_meas_two")
...
discrimreg_final_wf <- 
  ...
  finalize_workflow(discrimreg_wf, .)

# Finalise model
discrimreg_final_model <- 
  discrimreg_final_wf %>% 
  last_fit(split)

# Save final model
# readr::write_rds(discrimreg_final_model, "full_data_analysis_7Dec2025/final_models/30features/discrimreg_mod.rds")
