...
dat_train_baked <- rf_rec %>% prep() %>% juice()
...
tictoc::toc() # 16899.061 sec elapsed
beepr::beep(3)

# Save CV result
# readr::write_rds(rf_tuned, "full_data_analysis_7Dec2025/cv_results/30_features/rf_cv_result.rds")
...
# Plot
autoplot(rf_tuned)
autoplot(rf_tuned, metric = "sens")
autoplot(rf_tuned, metric = "spec")
autoplot(rf_tuned, metric = "pr_auc")
autoplot(rf_tuned, metric = "f_meas_two")
...
# Save final model
# readr::write_rds(rf_final_model, "full_data_analysis_7Dec2025/final_models/30_features/rf_mod.rds")
